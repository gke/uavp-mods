        ...
        */
        ...
        public void SetSerialPort(string portSelected, int speed, ref string errorMessage)
        {
            try
            {
                ...
                serialPort2.ReadBufferSize = 1024;
        ...
        public Form1()
        {
           ...
           // headingIndicatorInstrumentControl1.SetHeadingIndicatorParameters(45);
            ...
            foreach (string x in update_available_ports())
            {
        ...
        void UAVXRead()
        {
            ...
            SetSerialPort(Convert.ToString(cboComSelect.SelectedItem), 38400, ref sError);
            ...
            BatteryFailS.Text = Convert.ToString(ExtractShort(ref Stats, 34));
            if (ExtractShort(ref Stats, 34) > 0)
                BatteryFailS.BackColor = System.Drawing.Color.Orange;
            else
                BatteryFailS.BackColor = GPSStatsGroupBox.BackColor;
        ...
        private void MenuSaveLog_Click(object sender, EventArgs e)
        {
            ...
            dlg.DefaultExt = "txt";
            dlg.InitialDirectory = Application.StartupPath + "\\Logs";
            dlg.Title = "Log File";
            dlg.AddExtension = true;
            dlg.Filter = "Log Files (*.txt)|*.txt|All files (*.*)|*.*";
        ...
        private void MenuSaveMission_Click(object sender, EventArgs e)
        {
            ...
            if (dlg.ShowDialog() == DialogResult.OK)
            {
                ...
                sw.WriteLine("OPTIONS:" + Convert.ToInt16(Convert.ToDouble(ProximityRadius.Text))
                ...
                sw.WriteLine("HOME:" + Regex.Replace(Convert.ToString(LaunchLat.Text), 
                    ",", ".") + "," + Regex.Replace(Convert.ToString(LaunchLon.Text), 
                    ",", ".") + "," + Convert.ToInt16(Convert.ToDouble(RTHAltitude.Text
                    )));
                for (nCount = 1;nCount <= M.Rows.Count;nCount++)
                    sw.WriteLine(Regex.Replace(Convert.ToString(M[1,nCount-1].Value), 
        ...
        private void UAVXOpenTelemetry()
        {
            ...
            if (serialPort2.IsOpen)
                serialPort2.Close();
            SetSerialPort(Convert.ToString(cboComSelect.SelectedItem), 9600, ref sError);
            if (sError != "")
            {
                InFlight = false;
        ...
        void PollTelemetry()
        {
            while (serialPort2.BytesToRead != 0)
            {
                RxTail++;
                RxTail &= RxQueueMask;
                RxQueue[RxTail] = (byte)serialPort2.ReadByte();
            }
            UAVXReadTelemetry();
        }
                ...
                if (PacketReceived)
                {
                    ...
                    switch (RxPacketTag)
                    {
                        case UAVXFlightPacketTag:
                            ...
                            */
                            if ((Flags[0] & 0x20) != 0)
                            {
                                ...
                                BatteryFailS.BackColor = System.Drawing.Color.Red;
                            }
                            else
                            {
                                BatteryVolts.BackColor = BatteryGroupBox.BackColor;
                                BatteryFailS.BackColor = ErrorStatsGroupBox.BackColor;
                            }
                            if ((Flags[0] & 0x40) != 0)
                        ...
                        case UAVXNavPacketTag:
                            ...
                            MinBaroROCS.Text = string.Format("{0:n1}", (float)BaroROCT * 0.01);
                            RelBaroAltitudeS.Text = string.Format("{0:n1}", (float)RelBaroAltitudeT * 0.01);
