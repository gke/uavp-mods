...
%LEXE% %LCMD% %F% /u_CRUNTIME /z__MPLAB_BUILD=1 /W /o UAVXRazor%L%-V1.$WCREV$gke-%E%%PROC%%X%%R%%C%%D%%T%.hex >> log.lst 


if %ERRORLEVEL% == 1 goto FAILED

echo compiled - UAVXRazor%L%-V1.$WCREV$gke-%E%%PROC%%X%%R%%C%%D%%T%.hex
echo compiled - UAVXRazor%L%-V1.$WCREV$gke-%E%%X%%R%%C%%D%%T%.hex >> gen.lst
call makeclean.bat
goto FINISH

:FAILED
echo failed - UAVXRazor%L%-V1.$WCREV$gke-%E%%PROC%%X%%R%%C%%D%%T%.hex
echo failed - UAVXRazor%L%-V1.$WCREV$gke-%E%%PROC%%X%%R%%C%%D%%T%.hex >> gen.lst
