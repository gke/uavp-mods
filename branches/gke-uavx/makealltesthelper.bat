...
set CSRC=adc irq uavx-test compass_altimeter accel serial i2c menu-test tests utils gps autonomous
set ASRC=bootl18f
...
%LEXE% %LCMD% %F% /u_CRUNTIME /z__MPLAB_BUILD=1 /W /o UAVXTest-V%VERSION%%PROC%%X%-%T%%G%%R%%E%.hex >> log.lst 


if %ERRORLEVEL% == 1 goto FAILED

echo compiled - UAVXTest-V%VERSION%%PROC%%X%-%D%%T%%G%%R%%E%.hex
echo compiled - UAVXTest-V%VERSION%%PROC%%X%-%D%%T%%G%%R%%E%.hex >> gen.lst
call makeclean.bat
goto FINISH

:FAILED
echo failed - UAVXTest-V%VERSION%%PROC%%X%-%T%%G%%R%%E%.hex
echo failed - UAVXTest-V%VERSION%%PROC%%X%-%T%%G%%R%%E%.hex >> gen.lst
