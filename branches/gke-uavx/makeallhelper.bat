...
%LEXE% %LCMD% %F% /u_CRUNTIME /z__MPLAB_BUILD=1 /W /o UAVX-V1.273gke-%PROC%%X%%R%%C%%D%%T%.hex >> log.lst 


if %ERRORLEVEL% == 1 goto FAILED

echo compiled - UAVX-V1.273gke-%PROC%%X%%R%%C%%D%%T%.hex
echo compiled - UAVX-V1.273gke-%PROC%%X%%R%%C%%D%%T%.hex >> gen.lst
call makeclean.bat
goto FINISH

:FAILED
echo failed - UAVX-V1.273gke-%PROC%%X%%R%%C%%D%%T%.hex
echo failed - UAVX-V1.273gke-%PROC%%X%%R%%C%%D%%T%.hex >> gen.lst
