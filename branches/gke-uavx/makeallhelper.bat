...
%LEXE% %LCMD% %F% /u_CRUNTIME /z__MPLAB_BUILD=1 /W /o UAVX-V1.407gke-%PROC%%X%_%C%%D%%T%%G%%R%%E%.hex >> log.lst 


if %ERRORLEVEL% == 1 goto FAILED

echo compiled - UAVX-V1.407gke-%PROC%%X%_%C%%D%%T%%G%%R%%E%.hex
echo compiled - UAVX-V1.407gke-%PROC%%X%_%C%%D%%T%%G%%R%%E%.hex >> gen.lst
call makeclean.bat
goto FINISH

:FAILED
echo failed - UAVX-V1.407gke-%PROC%%X%_%C%%D%%T%%G%%R%%E%.hex
echo failed - UAVX-V1.407gke-%PROC%%X%_%C%%D%%T%%G%%R%%E%.hex >> gen.lst
